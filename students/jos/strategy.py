...
class Strategy(OthelloCore.OthelloCore):
    ...
    def advanced_make_move(self, move, player,
    ...
    def alternative_minimax(self, board, player, max_d, a, b):
        if player == OthelloCore.BLACK:
            ...
            result = self.alternative_max_dfs(board, player, max_d, 0, a, b, None, 60 - self.open_squares(board))
            return result
        if player == OthelloCore.WHITE:
            # pickle.dump(boardToValue, fout, protocol=2)
            result = self.alternative_min_dfs(board, player, max_d, 0, a, b, None, 60 - self.open_squares(board))
            return result
    ...
    def scoreBoard(self, board, player):
        ...
        for i in range(11, 90):
            if board[i] == OthelloCore.BLACK:
                ...
                total -= self.SQUARE_WEIGHTS[i]
        # maxmoves = len(self.advanced_legal_moves(self.MAX, board, 2))
        # minmoves = len(self.advanced_legal_moves(self.MIN, board, 2))
        # total += 5*(maxmoves-minmoves)/(maxmoves + minmoves)
        return total
    ...
    def sort(self, legalMoves, board, player, max):
        newlist = []
        for move in legalMoves:
            m = move[1]
            # print(m)
            if max == False:
                currentMin = self.table.get(str(self.make_move(m, player, board[:])) + 'currentMin', None)
                if currentMin is not None:
                    # print ('h1')
                    newlist.append((currentMin[2], m))
                else:
                    newlist.append((self.SQUARE_WEIGHTS[m], m))
            else:
                currentMax = self.table.get(str(self.make_move(m, player, board[:])) + 'currentMax', None)
                if currentMax is not None:
                    # print('h2')
                    newlist.append((currentMax[3], m))
                else:
                    newlist.append((self.SQUARE_WEIGHTS[m], m))
        return newlist

    def alternative_max_dfs(self, board, player, max_d, current_d, alpha, beta, myLegalMoves, age):
        # self.timeSpentInLegalMoves += 1
        currentMin = self.table.get(str(board) + 'currentMin', None)
        currentMax = self.table.get(str(board) + 'currentMax', None)
        if currentMin is not None:
            if currentMin[2] >= beta and currentMin[4] >= age + max_d - current_d:
                # print ("here1")
                return (currentMin[0], currentMin[1], currentMin[2], currentMin[3])
            elif currentMin[4] >= age + max_d - current_d:
                alpha = max(alpha, currentMin[2])  # if 0 loses, try 2/3 for max, 2, min, 3
        if currentMax is not None:
            if currentMax[3] <= alpha and currentMax[4] >= age + max_d - current_d:
                # print ("here2")
                return (currentMax[0], currentMax[1], currentMax[2], currentMax[3])
            elif currentMax[4] >= age + max_d - current_d:
        ...
        if myLegalMoves == None:
            ...
            # print(myLegalMoves)
        # myLegalMoves = self.sort(myLegalMoves, board, player, True)
        _heapify_max(myLegalMoves)
        # myLegalMoves = myLegalMoves[::-1]
        for m in myLegalMoves:
            if self.is_valid(m[1]) == False:
                continue
            boardCopyWithMove = self.advanced_make_move(m[1], player, board.copy())
        ...
        if v >= beta:
            if str(board) + 'currentMin' in self.table:
                if self.table[str(board) + 'currentMin'][4] <= age + max_d - current_d:
                    self.table[str(board) + 'currentMin'] = v, move, v, beta, age + max_d - current_d
            else:
                self.table[str(board) + 'currentMin'] = v, move, v, beta, age + max_d - current_d
    ...
    def alternative_min_dfs(self, board, player, max_d, current_d, alpha, beta, myLegalMoves, age):
        # self.timeSpentInLegalMoves += 1
        currentMin = self.table.get(str(board) + 'currentMin', None)
        currentMax = self.table.get(str(board) + 'currentMax', None)
        if currentMax is not None:
        ...
        if myLegalMoves == None:
            myLegalMoves = self.advanced_legal_moves(player, board, current_d)
            # print(myLegalMoves)
        # myLegalMoves = self.sort(myLegalMoves, board, player, False)
        _heapify_max(myLegalMoves)
        mininloop = False
        # myLegalMoves = myLegalMoves[::-1]
        for m in myLegalMoves:
        ...
        if v <= alpha:
            if str(board) + 'currentMax' in self.table:
                if self.table[str(board) + 'currentMax'][4] <= age + max_d - current_d:
                    self.table[str(board) + 'currentMax'] = v, move, alpha, v, age + max_d - current_d
            else:
                self.table[str(board) + 'currentMax'] = v, move, alpha, v, age + max_d - current_d
        '''if v > alpha and v < beta:
            print ("wrong")
    ...
    def advanced_legal_moves(self, player, board, current_d):
        if (player, str(board)) in self.dictionary:
            ...
            self.timeSpentInLegalMoves += time.time() - startTime
            return legalMoves
